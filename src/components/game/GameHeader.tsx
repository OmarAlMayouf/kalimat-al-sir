...
const GameHeader = ({ game, onEndTurn }: GameHeaderProps) => {
    ...
    return (
        <div className="bg-card/80 backdrop-blur-sm border-b border-border/50 px-4 py-2.5 sticky top-0 z-20">
          ...
          <span className={`shrink-0 text-xs font-bold px-3 py-1.5 rounded-full transition-colors ${
              isRed
            ...
            {isRed ? 'الأحمر' : 'الأزرق'}
                    ...
                    {game.currentHint ? (
                        <div className="flex items-center gap-2 bg-secondary/80 rounded-xl px-3 py-1.5 min-w-0 overflow-hidden">
                            <span className="font-bold text-sm text-foreground truncate">{game.currentHint.word}</span>
                            <span className="text-xs text-muted-foreground shrink-0">({game.currentHint.count})</span>
                            {isGuessing && (
              ...
              {game.turnPhase === 'hint' ? 'في انتظار التلميح...' : ''}
                ...
                {/* Timer */}
                {isGuessing && (
                    <div className={`flex items-center gap-1.5 shrink-0 font-mono font-bold text-lg tabular-nums transition-colors ${
                        isLowTime ? 'text-destructive animate-pulse' : 'text-foreground'
                    }`}>
                        <Timer className={`w-4 h-4 ${isLowTime ? 'text-destructive' : 'text-muted-foreground'}`} />
