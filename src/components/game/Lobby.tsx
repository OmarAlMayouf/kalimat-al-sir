        ...
        <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4 py-8 relative overflow-hidden">
            ...
            <div className="w-full max-w-xl mx-auto animate-fade-in space-y-5 relative z-10">
                ...
                <div className="text-center space-y-1">
                    ...
                    <p className="text-muted-foreground text-sm">اختر فريقك وانتظر بدء المضيف</p>
                </div>

                {/* Room code card */}
                <div className="bg-card border border-border/60 rounded-2xl p-5 shadow-sm space-y-4">
                    <p className="text-xs text-muted-foreground text-center font-medium uppercase tracking-widest">كود الغرفة</p>
            ...
            <span className="text-4xl font-mono font-black tracking-[0.25em] text-primary select-all">
              {game.roomCode}
                    ...
                    <Button variant="outline" size="sm" onClick={shareWhatsApp} className="gap-2 w-full">
                ...
                {redTeam.length}
                        ...
                        <div className="space-y-1.5 min-h-[60px]">
                            {redTeam.length === 0 && (
                                <p className="text-[11px] text-muted-foreground text-center py-3">لا يوجد لاعبون</p>
                            )}
                            {redTeam.map(p => (
                                <div
                                    key={p.id}
                                    className={`rounded-xl px-3 py-2 text-xs flex items-center justify-between gap-1 transition-colors ${
                                        p.id === playerId ? 'bg-team-red/20 border border-team-red/30' : 'bg-background/60'
                                    }`}
                                >
                                    <div className="flex items-center gap-1.5 min-w-0">
                                        {p.isSpymaster && <Crown className="w-3 h-3 text-gold flex-shrink-0" />}
                                    ...
                                    {p.isSpymaster && (
                                        <span className="text-[9px] bg-gold/20 text-gold px-1.5 py-0.5 rounded-full font-bold flex-shrink-0">رئيس</span>
                                    )}
                                </div>
                            ))}
                        </div>
                    ...
                    <div className={`rounded-2xl p-4 border transition-all duration-200 ${
                        ...
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="text-sm font-bold text-team-blue">الفريق الأزرق</h3>
                        ...
                        <div className="space-y-1.5 min-h-[60px]">
                            ...
                            {blueTeam.map(p => (
                                <div
                                    key={p.id}
                                    className={`rounded-xl px-3 py-2 text-xs flex items-center justify-between gap-1 transition-colors ${
                                        p.id === playerId ? 'bg-team-blue/20 border border-team-blue/30' : 'bg-background/60'
                                    }`}
                                >
                                    <div className="flex items-center gap-1.5 min-w-0">
                                        {p.isSpymaster && <Crown className="w-3 h-3 text-gold flex-shrink-0" />}
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                    <div className={`flex items-center gap-1.5 justify-center py-2 px-3 rounded-xl ${redHasSpymaster && redHasPlayers ? 'bg-green-500/10 text-green-600 dark:text-green-400' : 'bg-muted/50 text-muted-foreground'}`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${redHasSpymaster && redHasPlayers ? 'bg-green-500' : 'bg-muted-foreground/40'}`} />
                        الأحمر {redHasSpymaster && redHasPlayers ? 'جاهز ✓' : 'غير جاهز'}
                    </div>
                    <div className={`flex items-center gap-1.5 justify-center py-2 px-3 rounded-xl ${blueHasSpymaster && blueHasPlayers ? 'bg-green-500/10 text-green-600 dark:text-green-400' : 'bg-muted/50 text-muted-foreground'}`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${blueHasSpymaster && blueHasPlayers ? 'bg-green-500' : 'bg-muted-foreground/40'}`} />
                ...
                >
                    ...
                    {currentPlayer?.isSpymaster ? 'إلغاء دور رئيس الفريق' : 'أكون رئيس الفريق'}
                </button>
